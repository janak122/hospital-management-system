...
<c:set var="destinations"
	value="${EntitiesConstants.PATIENT}/1;;${EntitiesConstants.ROOM}/1;;${EntitiesConstants.DOCTOR}/1;;${EntitiesConstants.STAFF}/1" />
<c:set var="links" value="Patients;;Rooms;;Doctors;;Staff" />
		...
		<div class="collapse navbar-collapse" id="navbarResponsive">
			<ul class="navbar-nav ml-auto">
				<c:forEach var="dest" items="${destinations}" varStatus="status">
					<li class="nav-item"><a
						href="${pageContext.request.contextPath}/receptionist/list/${dest}"
						class="nav-link">${links[status.index]}</a></li>
				</c:forEach>
				<li class="nav-item"><a
					href="${pageContext.request.contextPath}/logout" class="nav-link">LogOut</a></li>
			</ul>
		</div>
	</div>
</nav>
